<template>
    <div class="bg-[#1b1a1a] px-6 pt-5">
        <div class="w-screen lg:w-3/5">
            <ClientOnly>
                <VideoPlayer :episodeId="episodeId" :poster="poster" class="mb-5" />
            </ClientOnly>
            <div class="flex justify-between">
                <USelect v-model="selectValue" :options="selectOption" class="w-36 text-white mb-5" color="rose"
                    variant="outline" option-attribute="name" :ui="{
                        color: {
                            rose: {
                                outline:
                                    'text-[red] bg-[#2a2a2a] focus:ring-0 dark:focus:ring-0',
                            },
                        },
                        gap: 'gap-y-1',
                    }" />
                <UButton class="h-11 flex justify-center bg-[#ea4646] hover:bg-[#c34242]" disable icon="i-mdi-youtube"
                    size="md">Trailer</UButton>
            </div>
            <div class="flex gap-3 flex-wrap w-full">
                <div v-for="(episode, index) in currentEpisodes" :key="index">
                    <UButton :label="`${episode.number}`" variant="outline" @click="changeEpisode(episode.id)" color="rose"
...
const poster = ref("");
const { data: animeEpisodes } = await useFetch(
...
watch(
    ...
    () => {
        let arr = [];
        for (
            let i = selectValue.value * 100;
            i < animeEpisodes.value.episodes.length &&
            i < selectValue.value * 100 + 100;
            i++
        ) {
